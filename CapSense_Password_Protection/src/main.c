...
void transferComplete(unsigned int channel, bool primary, void *user)
{
	...
	circBufferPush(&circbuff_t,"MEHERJ",(LEUART_TX_SIZE));
	//for (i=0;i<12;i++)
		//circBufferPop();
	LEUART0->IEN |=LEUART_IEN_TXBL;
	//circBufferPush(&circbuff_t,"SHIVAS",LEUART_TX_SIZE);
	//leuart_tx(0xaa);															// Enable the LEUART TXBL interrupt
}
...
void disableLESENSE(void)
{
	ACMP_Disable(ACMP0);
	ACMP_Disable(ACMP1);
	GPIO_PinOutClear(CAPLESENSE_SLIDER_PORT0,CAPLESENSE_SLIDER0_PIN);
	GPIO_PinOutClear(CAPLESENSE_SLIDER_PORT0,CAPLESENSE_SLIDER1_PIN);
	GPIO_PinOutClear(CAPLESENSE_SLIDER_PORT0,CAPLESENSE_SLIDER2_PIN);
	GPIO_PinOutClear(CAPLESENSE_SLIDER_PORT0,CAPLESENSE_SLIDER3_PIN);
	SegmentLCD_Disable();
	LESENSE_ScanStop();
