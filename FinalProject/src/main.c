 ...
 * Date: 09/18/2016
 * Description: ECEN 5023 Final Project*/
...
static volatile CAPSENSE_STATES capSenseState = CAPSENSE_SLEEP_PREPARE;
...
#if defined(DMA_ON_ADC)																					// If using DMA
...
#if !defined (DMA_ON_ADC)													// If DMA is not used enable the ADC interrupts
...
void dmaConfig_ADC(void)
{
	...
	descrCfg.arbRate	 = dmaArbitrate1;													    // No arbritration, since, ADC is only peripheral using DMA
...
void disableLESENSE(void)
{
	ACMP_Disable(ACMP0);
	ACMP_Disable(ACMP1);
	LESENSE_ScanStop();
	//SegmentLCD_Disable();
	LESENSE_ChannelEnable(CAPLESENSE_SLIDER0_PIN,false, false);
	LESENSE_ChannelEnable(CAPLESENSE_SLIDER1_PIN,false, false);
	LESENSE_ChannelEnable(CAPLESENSE_SLIDER2_PIN,false, false);
	LESENSE_ChannelEnable(CAPLESENSE_SLIDER3_PIN,false, false);

	/* Disable HF peripheral clock. */
  //CMU_ClockEnable(cmuClock_HFPER, 0);
  /* Disable clock for GPIO. */
  CMU_ClockEnable(cmuClock_GPIO, 0);
  /* Disable clock for ACMP0. */
  CMU_ClockEnable(cmuClock_ACMP0, 0);
  /* Disable clock for ACMP1. */
  CMU_ClockEnable(cmuClock_ACMP1, 0);
  /* Disable CORELE clock. */
 // CMU_ClockEnable(cmuClock_CORELE, 0);
  /* Disable clock for LESENSE. */
  CMU_ClockEnable(cmuClock_LESENSE, 0);
...
{
	capSenseState = CAPSENSE_PREPARE;
...
void passwordCheck()
{
	...
	while(1)
	{
		...
		{
			case CAPSENSE_SLEEP_PREPARE:
			{
				CAPLESENSE_setupLESENSE(true);
				SegmentLCD_Disable();
				capSenseState = CAPSENSE_SLEEP;
			}
			break;
			...
			case CAPSENSE:
			{
				...
				if(sliderPos != -1)
				{
					if((sliderPos > PAD4_SLIDER_THRESHOLD) && (!debounce))
					{
						debounce = true;
						password += sliderPos;
						if(password>=PASSWORD_THRESHOLD)
						{
							SegmentLCD_Write("VERIFIED");
							passwordSuccess = true;
						}
					}
					if((sliderPos < PAD1_SLIDER_THRESHOLD) && (debounce))
					{
						debounce = false;
					}
				}
...
#if defined(DMA_ON_ADC)													// If DMA is used
